            ...
            for i in range(len(veh_route)):
                ...
                if len(veh_route) > 1 and i != len(veh_route) - 1:
                    ss_node = self.data['service_stations'].index[np.argmin(
                        [self.data['travel_distance'][customer_dest][ss] + self.data['travel_distance'][ss][veh_route[i + 1]]
                         for ss in self.data['service_stations'].index])]
                else:
                    ss_node = self.data['service_stations'].index[np.argmin([self.data['travel_distance'][customer_dest][ss]
                         for ss in self.data['service_stations'].index])]
