...
class SinglePoint(Crossover):
    ...
    def _do(self, problem, X, **kwargs):
        ...
        for k in range(n_matings):
            ...
            # TODO: FIXUP to ensure correct number of 0's and that all customers are served.
            all_customers = self.demand.sort_values('TimeWindow').index
            for chromosome in [a, b]:
                ...
                extra_zeros = []
                if sum(chromosome == 0) > problem.n_vehicles - 1:
                    num_extra_zeros = sum(chromosome == 0) - (problem.n_vehicles - 1)
                    extra_zeros = [zero_idx for zero_idx in np.where(chromosome == 0)[0] if zero_idx >= co_point][:num_extra_zeros]
                if sum(chromosome == 0) < problem.n_vehicles - 1:
                    missing_customers += [0] * (problem.n_vehicles - sum(chromosome == 0) - 1)
            ...
            assert sum(a == 0) == problem.n_vehicles - 1
            assert sum(b == 0) == problem.n_vehicles - 1
            Y[0, k], Y[1, k] = a, b
        return Y